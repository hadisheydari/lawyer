...
<style>
    ...
    .article-card { background:#fff; border-radius:14px; overflow:hidden; box-shadow:0 4px 15px rgba(0,0,0,0.05); border:1px solid #f0f0f0; transition:0.3s; }
    .article-card:hover { transform:translateY(-3px); border-color:var(--gold-main); }

    .ac-image { height:160px; background:linear-gradient(135deg,var(--navy),#1e3a5f); display:flex; align-items:center; justify-content:center; position:relative; overflow:hidden; }
    .ac-image img { width:100%; height:100%; object-fit:cover; }
    .ac-image-placeholder { font-size:3rem; color:rgba(212,175,55,0.3); }
    .ac-status { position:absolute; top:12px; right:12px; }
    ...
    .ac-cat { font-size:0.72rem; color:var(--gold-dark); font-weight:700; margin-bottom:6px; }
    .ac-title { font-size:0.95rem; font-weight:800; color:var(--navy); margin-bottom:8px; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; }
    .ac-meta { font-size:0.75rem; color:#888; display:flex; gap:10px; flex-wrap:wrap; margin-bottom:14px; }
    .ac-meta span { display:flex; align-items:center; gap:4px; }

    .ac-footer { display:flex; gap:8px; justify-content:space-between; align-items:center; }
    .btn-sm { padding:6px 13px; border-radius:7px; font-size:0.78rem; font-weight:700; text-decoration:none; display:inline-flex; align-items:center; gap:4px; transition:0.2s; }
    ...
    .btn-del { background:#fee2e2; color:#b91c1c; border:none; cursor:pointer; font-family:'Vazirmatn',sans-serif; }
