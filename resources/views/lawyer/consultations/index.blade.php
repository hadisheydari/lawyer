...
@push('styles')
    <style>
        ...
        .table-box {
            background: #fff; border-radius: 14px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.05); overflow: hidden;
        }

        .data-table { width: 100%; border-collapse: collapse; }
        .data-table th {
            background: #f8fafc; padding: 13px 18px; text-align: right;
            font-size: 0.82rem; color: #64748b; font-weight: 700; border-bottom: 1px solid #f0f0f0;
        }
        .data-table td {
            padding: 14px 18px; border-bottom: 1px solid #f8f8f8;
            font-size: 0.88rem; color: #374151; vertical-align: middle;
        }
        ...
        .badge-cancelled, .badge-rejected { background: #fee2e2; color: #b91c1c; }
        ...
        @media (max-width: 768px) {
            ...
            .data-table tr {
                ...
                margin-bottom: 15px; padding: 12px; position: relative;
            }
            .data-table td {
                display: flex; justify-content: space-between; align-items: center;
                border-bottom: 1px dashed #e2e8f0; padding: 10px 5px; text-align: left;
            }
            .data-table td:last-child { border-bottom: none; }
            
            /* ایجاد عنوان در حالت موبایل با استفاده از data-label */
...
@section('content')
    ...
    <div class="stats-grid">
        <div class="stat-card pending">
            <span class="stat-n">{{ $stats['pending'] ?? 0 }}</span>
            <span class="stat-l">در انتظار تأیید</span>
        </div>
        <div class="stat-card confirmed">
            <span class="stat-n">{{ $stats['confirmed'] ?? 0 }}</span>
            <span class="stat-l">تأیید شده</span>
        </div>
        <div class="stat-card completed">
            <span class="stat-n">{{ $stats['completed'] ?? 0 }}</span>
            <span class="stat-l">تکمیل شده</span>
        </div>
        <div class="stat-card cancelled">
            <span class="stat-n">{{ $stats['cancelled'] ?? 0 }}</span>
    ...
    <form method="GET" class="filter-bar">
        <input type="text" name="search" placeholder="جستجو نام یا تلفن موکل..." value="{{ request('search') }}">
        <select name="status">
            ...
            <tbody>
                @forelse($consultations as $c)
                    ...
                    <tr>
                        ...
                        <td data-label="زمان">
                            @if ($c->scheduled_at)
                                @if ($c->scheduled_at->year > 1900)
                                    {{ \Morilog\Jalali\Jalalian::fromCarbon($c->scheduled_at)->format('Y/m/d') }}
                                @else
                                    {{ $c->scheduled_at->format('Y/m/d') }} (نیاز به اصلاح)
                                @endif 
                                <br><small>ساعت {{ $c->scheduled_at->format('H:i') }}</small>
                            @else
