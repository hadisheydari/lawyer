    ...
    <div class="grid-2">
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
            <div class="info-card">
                ...
                <div class="info-row"><span class="info-label">عنوان</span><span
                        class="info-value">{{ $consultation->title }}</span></div>
                <div class="info-row"><span class="info-label">نوع</span><span
                        class="info-value">{{ $typeLabels[$consultation->type] ?? $consultation->type }}</span></div>
                <div class="info-row"><span class="info-label">وضعیت</span><span
                        class="badge {{ $s['class'] }}">{{ $s['label'] }}</span></div>
                ...
                @if ($consultation->cancellation_reason)
                    <div class="info-row"><span class="info-label">دلیل لغو/رد</span><span class="info-value"
                            style="color:#b91c1c;">{{ $consultation->cancellation_reason }}</span></div>
                @endif
            </div>
            ...
            <div class="info-card">
                ...
                @if ($consultation->lawyer_notes)
                    <div class="note-box">
                        <p>{{ $consultation->lawyer_notes }}</p>
                    </div>
                @endif
                <form method="POST" action="{{ route('lawyer.consultations.note', $consultation) }}"
                    style="margin-top:14px;">
                    @csrf
                    <textarea name="lawyer_notes" rows="4"
                        style="width:100%;padding:12px;border:1.5px solid #e0e0e0;border-radius:8px;font-family:'Vazirmatn',sans-serif;font-size:0.9rem;resize:vertical;outline:none;"
                        placeholder="یادداشت خود را اینجا بنویسید...">{{ $consultation->lawyer_notes }}</textarea>
                    <button type="submit"
                        style="margin-top:10px;padding:10px 20px;background:var(--navy);color:#fff;border:none;border-radius:8px;font-family:'Vazirmatn',sans-serif;font-weight:700;cursor:pointer;">
        ...
        <div class="modal">
            ...
            <form method="POST" action="{{ route('lawyer.consultations.reject', $consultation) }}">
                ...
                <div class="modal-btns">
                    ...
                    <button type="submit"
                        style="padding:10px 18px;background:#b91c1c;color:#fff;border:none;border-radius:8px;cursor:pointer;font-family:'Vazirmatn',sans-serif;font-weight:700;">رد
                        کردن</button>
                </div>
            </form>
        </div>
        ...
        <div class="modal">
            ...
            <form method="POST" action="{{ route('lawyer.consultations.cancel', $consultation) }}">
                ...
                <div class="modal-btns">
                    ...
                    <button type="submit"
                        style="padding:10px 18px;background:#b45309;color:#fff;border:none;border-radius:8px;cursor:pointer;font-family:'Vazirmatn',sans-serif;font-weight:700;">لغو
                        مشاوره</button>
                </div>
            </form>
        </div>
