...
<style>
    ...
    .client-card { background:linear-gradient(135deg,var(--navy),#1e3a5f); border-radius:14px; padding:28px; color:#fff; text-align:center; }
    .client-avatar-lg { width:80px; height:80px; border-radius:50%; background:rgba(212,175,55,0.2); border:3px solid rgba(212,175,55,0.5); display:flex; align-items:center; justify-content:center; font-size:2rem; font-weight:900; color:var(--gold-main); margin:0 auto 14px; }
    .client-card h3 { font-size:1.1rem; font-weight:800; margin-bottom:4px; }
    .client-card p { color:rgba(255,255,255,0.6); font-size:0.82rem; }
    .badge-special-wt { background:rgba(212,175,55,0.2); border:1px solid rgba(212,175,55,0.4); color:var(--gold-main); padding:4px 14px; border-radius:20px; font-size:0.78rem; font-weight:700; display:inline-flex; align-items:center; gap:5px; margin-top:10px; }

    .info-card { background:#fff; border-radius:14px; padding:22px; box-shadow:0 4px 15px rgba(0,0,0,0.05); }
    ...
    .ms-item { text-align:center; background:#f8fafc; padding:14px; border-radius:10px; }
    .ms-item .n { font-size:1.4rem; font-weight:900; color:var(--navy); display:block; }
    .ms-item .l { font-size:0.72rem; color:#888; }

    .upgrade-btn { width:100%; padding:12px; background:linear-gradient(135deg,var(--gold-main),var(--gold-dark)); color:#fff; border:none; border-radius:10px; font-family:'Vazirmatn',sans-serif; font-weight:700; font-size:0.9rem; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px; transition:0.2s; margin-top:14px; }
    .upgrade-btn:hover { opacity:0.9; }

    .section-title { font-size:1rem; font-weight:800; color:var(--navy); margin-bottom:18px; display:flex; align-items:center; gap:8px; }
    .section-title i { color:var(--gold-main); }

    .consult-list { display:flex; flex-direction:column; gap:10px; }
    .consult-item { background:#fff; border-radius:12px; padding:16px; box-shadow:0 2px 8px rgba(0,0,0,0.04); display:flex; justify-content:space-between; align-items:center; gap:12px; border:1px solid #f0f0f0; transition:0.2s; }
    .consult-item:hover { border-color:var(--gold-main); }
    .ci-left { display:flex; align-items:center; gap:12px; }
    .ci-icon { width:40px; height:40px; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:1rem; }
    .ci-info h4 { font-size:0.88rem; font-weight:700; color:var(--navy); margin:0 0 3px; }
    .ci-info p { font-size:0.75rem; color:#888; margin:0; }

    .case-list { display:flex; flex-direction:column; gap:10px; }
    .case-item { background:#fff; border-radius:12px; padding:16px; box-shadow:0 2px 8px rgba(0,0,0,0.04); border:1px solid #f0f0f0; border-right:3px solid var(--gold-main); transition:0.2s; }
    .case-item:hover { transform:translateY(-2px); }
    .case-item .num { font-size:0.72rem; color:var(--gold-dark); font-weight:700; margin-bottom:4px; }
    .case-item h4 { font-size:0.9rem; font-weight:800; color:var(--navy); margin-bottom:6px; }
    .case-item .meta { font-size:0.75rem; color:#888; display:flex; gap:12px; flex-wrap:wrap; }
    ...
    .btn-sm { padding:7px 14px; background:var(--navy); color:#fff; border-radius:8px; font-size:0.78rem; font-weight:700; text-decoration:none; display:inline-flex; align-items:center; gap:5px; flex-shrink:0; transition:0.2s; }
    .btn-sm:hover { background:var(--gold-main); color:var(--navy); }

    .empty-msg { text-align:center; padding:30px; color:#aaa; font-size:0.85rem; background:#fff; border-radius:12px; }
    .empty-msg i { font-size:2rem; display:block; margin-bottom:10px; opacity:0.3; }
    ...
    <div class="profile-sidebar">
        ...
        <div class="info-card">
            ...
            <div class="info-row">
                ...
                <span class="info-value">{{ \Morilog\Jalali\Jalalian::fromCarbon($client->created_at)->format('Y/m/d') }}</span>
            </div>
            @if($client->upgraded_at)
                <div class="info-row">
                    <span class="info-label">ارتقا به ویژه</span>
                    <span class="info-value">{{ \Morilog\Jalali\Jalalian::fromCarbon($client->upgraded_at)->format('Y/m/d') }}</span>
                </div>
            @endif
        ...
        @if($cases->isNotEmpty())
            <div class="case-list" style="margin-bottom:30px;">
                @foreach($cases as $case)
                    @php
                        $statusMap = ['active'=>['l'=>'فعال','c'=>'badge-active'],'on_hold'=>['l'=>'معلق','c'=>'badge-pending'],'closed'=>['l'=>'بسته','c'=>'badge-closed'],'won'=>['l'=>'برنده','c'=>'badge-won'],'lost'=>['l'=>'بازنده','c'=>'badge-lost']];
                    ...
                    <div class="case-item">
                        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;">
                            <div>
                                ...
                                <div class="meta">
                                    ...
                                    <span><i class="fas fa-money-bill-wave"></i> حق‌الوکاله: {{ number_format($case->total_fee) }} ت</span>
                        ...
                        @if($case->statusLogs->first())
                            <div style="margin-top:10px;padding-top:10px;border-top:1px solid #f0f0f0;font-size:0.78rem;color:#888;">
        ...
        @if($consultations->isNotEmpty())
            <div class="consult-list">
                @foreach($consultations as $c)
                    @php
                        $statusMap2 = ['pending'=>['l'=>'در انتظار','c'=>'badge-pending'],'confirmed'=>['l'=>'تأیید شده','c'=>'badge-confirmed'],'completed'=>['l'=>'تکمیل شده','c'=>'badge-completed'],'cancelled'=>['l'=>'لغو شده','c'=>'badge-cancelled'],'rejected'=>['l'=>'رد شده','c'=>'badge-cancelled']];
                        $s2 = $statusMap2[$c->status] ?? ['l'=>$c->status,'c'=>''];
                        $typeIcons = ['appointment'=>'fa-calendar-check','call'=>'fa-phone','chat'=>'fa-comment'];
                    @endphp
