                ...
                @if ($client->upgraded_at)
                    <div class="info-row">
                        ...
                        <span class="info-value">
                            @if ($client->upgraded_at->year > 1900)
                                {{ \Morilog\Jalali\Jalalian::fromCarbon($client->upgraded_at)->format('Y/m/d') }}
                            @else
                                {{ $client->upgraded_at->format('Y/m/d') }} (نیاز به اصلاح)
                            @endif{{ $client->upgraded_at->format('H:i') }}
                        </span>
                    </div>
                @endif
                                ...
                                <div style="flex:1;min-width:0;">
                                    ...
                                    <div class="meta">
                                        <span><i class="fas fa-calendar-alt"></i>
                                            @if ($case->opened_at->year > 1900)
                                                {{ \Morilog\Jalali\Jalalian::fromCarbon($case->opened_at)->format('Y/m/d') }}
                                            @else
                                                {{ $case->opened_at->format('Y/m/d') }} (نیاز به اصلاح)
                                            @endif
                            ...
                            @if ($case->statusLogs->first())
                                <div
                                    ...
                                    @if ($case->statusLogs->first()->status_date)
                                        —
                                        @if ($case->statusLogs->first()->status_date->year > 1900)
                                            {{ \Morilog\Jalali\Jalalian::fromCarbon($case->statusLogs->first()->status_date)->format('Y/m/d') }}
                                        @else
                                            {{ $case->statusLogs->first()->status_date->format('Y/m/d') }} (نیاز به اصلاح)ط
                                        @endif
                                    @endif
                                </div>
                            @endif
                                    ...
                                    <p>
                                        @if ($c->scheduled_at)
                                            @if ($c->scheduled_at->year > 1900)
                                                {{ \Morilog\Jalali\Jalalian::fromCarbon($c->scheduled_at)->format('Y/m/d H:i') }}
                                            @else
                                               ساعت   {{ $c->scheduled_at->format('Y/m/d H:i') }} (نیاز به اصلاح)
                                            @endif
