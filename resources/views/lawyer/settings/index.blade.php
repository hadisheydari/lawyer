...
@push('styles')
    <style>
        ...
        .card {
            ...
            margin-bottom: 25px;
            border-top: 6px solid var(--gold-main) !important;
        }
        ...
        .day-check-wrap input[type="checkbox"] {
            width: 18px;
            height: 18px;
...
@section('content')
    <div class="form-wrapper">
        ...
        @if (session('success'))
            <div
                style="background:#ecfdf5;border:2px solid #a7f3d0;color:#065f46;padding:18px 25px;border-radius:12px;margin-bottom:25px;display:flex;align-items:center;gap:12px;font-weight:800; font-size: 0.95rem; box-shadow: 0 4px 10px rgba(16, 185, 129, 0.1);">
        ...
        <div class="settings-section active" id="sec-profile">
            <div class="card">
                ...
                <form method="POST" action="{{ route('lawyer.settings.profile') }}" enctype="multipart/form-data">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="tel" name="phone" class="form-input @error('phone') is-error @enderror"
                                value="{{ old('phone', $lawyer->phone) }}" required dir="ltr"
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="experience_years" class="form-input"
                                value="{{ old('experience_years', $lawyer->experience_years) }}" min="0"
                                max="60">
                        </div>
                    </div>

                    <x-tag-input name="specializations" label="تخصص‌ها" :values="old(
                        'specializations',
                        is_array($lawyer->specializations) ? $lawyer->specializations : [],
                    )"
                        placeholder="مثال: حقوق خانواده — Enter بزنید" />
                    <div class="form-group">
                        <label class="form-label">بیوگرافی</label>
                        <textarea name="bio" class="form-input" rows="5" placeholder="معرفی کوتاه از تجربه و تخصص‌هایتان...">{{ old('bio', $lawyer->bio) }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        <label class="form-label">تصویر پروفایل <span
                                style="color:#94a3b8;font-weight:600;font-size:0.8rem;">(JPG/PNG — حداکثر
                                ۲MB)</span></label>
                        <input type="file" name="image" class="form-input" accept="image/jpeg,image/png"
                            style="padding:10px 18px !important; cursor: pointer;">
                    </div>
        ...
        <div class="settings-section" id="sec-availability">
            <div class="card">
                ...
                <p style="font-size:0.95rem;font-weight:600;color:#64748b;margin-bottom:25px;">مشخص کنید موکلین از چه
                    طریق‌هایی می‌توانند با شما ارتباط بگیرند.</p>

                <form method="POST" action="{{ route('lawyer.settings.availability') }}">
                    @csrf
                    {{-- دیگه نیازی به hidden name/phone نیست --}}

                    <div class="toggle-group">
                        <div class="toggle-label-wrap">
                            <strong><i class="fas fa-comment-dots" style="color:var(--gold-main);font-size:1.2rem;"></i>
                                گفتگوی آنلاین (چت)</strong>
                    ...
                    <div class="toggle-group">
                        <div class="toggle-label-wrap">
                            <strong><i class="fas fa-phone-alt" style="color:var(--gold-main);font-size:1.2rem;"></i> تماس
                                تلفنی</strong>
                    ...
                    <div class="toggle-group">
                        <div class="toggle-label-wrap">
                            <strong><i class="fas fa-calendar-check" style="color:var(--gold-main);font-size:1.2rem;"></i>
                                نوبت حضوری</strong>
        ...
        <div class="settings-section" id="sec-schedule">
            <div class="card">
                ...
                <p style="font-size:0.95rem;font-weight:600;color:#64748b;margin-bottom:25px;">روزها و ساعت‌های کاری خود را
                    مشخص کنید. اسلات‌های ۳۰ دقیقه‌ای بین ساعت شروع و پایان در دسترس موکلین قرار می‌گیرد.</p>
                        ...
                        @foreach ($days as $dayNum => $dayName)
                            ...
                            <div class="schedule-row">
                                <label class="day-check-wrap">
                                    <input type="hidden" name="schedules[{{ $dayNum }}][is_available]"
                                        value="0">
                                    <input type="checkbox" name="schedules[{{ $dayNum }}][is_available]"
                                        value="1" id="avail_{{ $dayNum }}"
                                        {{ $schedule && $schedule->is_available ? 'checked' : '' }}
                                        onchange="toggleDay({{ $dayNum }}, this.checked)">
                                    <span class="day-label">{{ $dayName }}</span>
                                </label>
                                <input type="hidden" name="schedules[{{ $dayNum }}][day_of_week]"
                                    value="{{ $dayNum }}">
                                <div class="time-inputs" id="times_{{ $dayNum }}"
        ...
        <div class="settings-section" id="sec-exceptions">
            <div class="card" id="calendar">
                ...
                <p style="font-size:0.95rem;font-weight:600;color:#64748b;margin-bottom:25px;">روزهای تعطیل اضافه یا روزهای
                    کاری خارج از برنامه هفتگی را اینجا ثبت کنید.</p>
                            ...
                            <div class="exception-item">
                                ...
                                <form method="POST" action="{{ route('lawyer.settings.exception.delete', $exc) }}">
                                    ...
                                    <button type="submit" class="btn-del"
                                        onclick="return confirm('این روز استثنا حذف شود؟')">
                ...
                @else
                    <div
                        style="text-align:center;padding:30px;color:#94a3b8;background:#f8fafc;border:2px dashed #e2e8f0;border-radius:12px;margin-bottom:25px;">
                        <i class="fas fa-calendar-check"
                            style="font-size:3rem;display:block;margin-bottom:15px;color:#cbd5e1;"></i>
                ...
                <div style="border-top:2px solid #f1f5f9;padding-top:25px;margin-top:15px;">
                    <div style="font-size:1.05rem;font-weight:900;color:var(--navy);margin-bottom:20px;">
                        <i class="fas fa-plus-circle" style="color:var(--gold-main);margin-left:8px;"></i>افزودن تاریخ
                        استثنا جدید
                    </div>
                    ...
                    <form method="POST" action="{{ route('lawyer.settings.exception.add') }}">
                        ...
                        <div class="add-exc-grid">
                            ...
                            <div class="form-group" style="margin-bottom:0;">
                                <label class="form-label">توضیحات <span
                                        style="color:#94a3b8;font-weight:600;">(اختیاری)</span></label>
                                <input type="text" name="reason" class="form-input"
                                    placeholder="مثال: مسافرت کاری، تعطیل رسمی...">
                            </div>
                        </div>
