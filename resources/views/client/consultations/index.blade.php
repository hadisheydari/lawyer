...
<style>
    ...
    .consult-list { display: flex; flex-direction: column; gap: 15px; }

    .consult-card {
        background: #fff; border-radius: 14px; padding: 22px 25px;
        box-shadow: 0 4px 15px rgba(0,0,0,0.05);
        border: 1px solid #f0f0f0;
        display: flex; justify-content: space-between; align-items: center;
        gap: 20px; transition: 0.3s;
    }
    .consult-card:hover { border-color: var(--gold-main); transform: translateY(-2px); }

    .cc-left { display: flex; align-items: center; gap: 16px; flex: 1; min-width: 0; }
    .cc-icon {
    ...
    .cc-meta { font-size: 0.8rem; color: #888; display: flex; gap: 12px; flex-wrap: wrap; }
    .cc-meta span { display: flex; align-items: center; gap: 4px; }

    .cc-right { display: flex; align-items: center; gap: 12px; flex-shrink: 0; }
    ...
    .empty-box i { font-size: 3rem; color: var(--gold-light); display: block; margin-bottom: 15px; }
    .empty-box p { color: #aaa; margin-bottom: 20px; }
    .btn-gold {
    ...
    .stat-mini-card .n { font-size: 1.6rem; font-weight: 800; color: var(--navy); display: block; }
    .stat-mini-card .l { font-size: 0.8rem; color: #888; margin-top: 3px; display: block; }

    @media (max-width: 600px) {
        .consult-card { flex-direction: column; align-items: flex-start; }
...
<div class="page-header">
    ...
    <a href="{{ route('reserve.index') }}" class="btn-gold" style="padding:10px 20px;border-radius:10px;font-size:0.88rem;">
    ...
    @forelse($consultations as $consultation)
        @php
            $typeMap = [
                ...
                'appointment' => ['icon' => 'fas fa-calendar-check', 'bg' => '#ede9fe', 'color' => '#6d28d9', 'label' => 'حضوری'],
            ];
            $t = $typeMap[$consultation->type] ?? ['icon' => 'fas fa-file', 'bg' => '#f3f4f6', 'color' => '#6b7280', 'label' => ''];
...
@if($consultations->hasPages())
    <div style="display:flex;justify-content:center;gap:8px;margin-top:30px;flex-wrap:wrap;">
        ...
        @else
            <a href="{{ $consultations->previousPageUrl() }}" style="padding:8px 14px;border-radius:8px;border:1px solid #ddd;color:var(--navy);text-decoration:none;">قبلی</a>
        @endif

        @foreach($consultations->getUrlRange(1, $consultations->lastPage()) as $page => $url)
            @if($page == $consultations->currentPage())
                <span style="padding:8px 14px;border-radius:8px;background:var(--navy);color:#fff;border:1px solid var(--navy);">{{ $page }}</span>
            @else
                <a href="{{ $url }}" style="padding:8px 14px;border-radius:8px;border:1px solid #ddd;color:var(--navy);text-decoration:none;">{{ $page }}</a>
            @endif
        @endforeach

        @if($consultations->hasMorePages())
            <a href="{{ $consultations->nextPageUrl() }}" style="padding:8px 14px;border-radius:8px;border:1px solid #ddd;color:var(--navy);text-decoration:none;">بعدی</a>
        @else
