...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar-title {
            font-size: 1.1rem; font-weight: 800; color: var(--gold-main);
            display: flex; align-items: center; gap: 10px;
        }

        .case-list { flex: 1; overflow-y: auto; padding: 15px 10px; }
        ...
        .case-item.active::before {
            ...
            width: 4px; background: var(--gold-main); border-radius: 4px 0 0 4px;
        }
        ...
        .case-info h4 { margin: 0 0 4px; font-size: 0.95rem; font-weight: 700; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .case-info p { margin: 0; font-size: 0.8rem; opacity: 0.7; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

        .unread-badge {
            background: #e74c3c; color: #fff; font-size: 0.7rem; font-weight: bold;
            padding: 2px 8px; border-radius: 20px; box-shadow: 0 2px 5px rgba(231,76,60,0.4);
        }
        ...
        .lawyer-profile { display: flex; align-items: center; gap: 15px; }
        .lawyer-img {
        ...
        .lawyer-details h3 { margin: 0 0 4px; font-size: 1.05rem; color: var(--navy); font-weight: 800; }
        .lawyer-details span { font-size: 0.8rem; color: var(--text-muted); display: flex; align-items: center; gap: 6px; }
        .online-dot { width: 8px; height: 8px; background: #2ecc71; border-radius: 50%; display: inline-block; box-shadow: 0 0 0 2px rgba(46,204,113,0.2); }

        .header-tools a {
            color: var(--text-muted); font-size: 1.1rem; padding: 10px; border-radius: 50%;
            transition: all 0.3s; text-decoration: none; display: inline-flex;
        }
        .header-tools a:hover { color: var(--navy); background: #f0f0f0; }
        ...
        .msg-row { display: flex; width: 100%; animation: fadeInMsg 0.4s ease forwards; }
        ...
        .msg-meta {
            ...
            margin-top: 8px; font-size: 0.7rem; opacity: 0.7;
        }
        .msg-row.sent .msg-meta { color: var(--gold-light); opacity: 1; }
        ...
        .msg-row.received .attachment { background: #f8f9fa; border: 1px solid #eee; }
        .attachment:hover { transform: translateY(-2px); }
        
        .file-icon {
            width: 40px; height: 40px; border-radius: 10px;
            background: rgba(197, 160, 89, 0.2); color: var(--gold-main);
            display: flex; align-items: center; justify-content: center; font-size: 1.2rem;
        }
        .msg-row.received .file-icon { background: rgba(44, 62, 80, 0.1); color: var(--navy); }
        
        .file-details { display: flex; flex-direction: column; }
        .file-name { font-weight: 700; font-size: 0.85rem; }
        .file-size { font-size: 0.7rem; opacity: 0.8; }
        ...
        .input-wrapper {
            ...
            padding: 5px 5px 5px 20px; transition: all 0.3s;
            border: 1px solid transparent;
        }
        .input-wrapper:focus-within {
            background: #fff; border-color: var(--gold-main);
            box-shadow: 0 5px 15px rgba(197, 160, 89, 0.1);
        }

        .msg-input {
            flex: 1; border: none; background: transparent;
            padding: 12px 0; font-family: 'Vazirmatn', sans-serif;
            font-size: 0.95rem; outline: none; color: var(--text-main);
        }
        .msg-input::placeholder { color: #a0aab5; }

        .btn-attach {
            color: #a0aab5; font-size: 1.2rem; cursor: pointer;
            padding: 10px; transition: 0.3s;
        }
        .btn-attach:hover { color: var(--gold-main); transform: scale(1.1); }
        ...
        .btn-send:hover { background: var(--gold-dark); transform: scale(1.05); box-shadow: 0 4px 10px rgba(197,160,89,0.3); }

        /* --- Empty State --- */
        .empty-chat {
            flex: 1; display: flex; flex-direction: column;
            align-items: center; justify-content: center; color: var(--text-muted); gap: 15px;
        }
        .empty-icon-wrap {
            width: 100px; height: 100px; border-radius: 50%;
            background: rgba(197, 160, 89, 0.1); color: var(--gold-main);
            display: flex; align-items: center; justify-content: center; font-size: 3.5rem;
        }

        /* --- Responsive --- */
        @media (max-width: 1024px) {
            .chat-layout { width: 100%; height: 100vh; border-radius: 0; grid-template-columns: 280px 1fr; }
        }
        @media (max-width: 768px) {
            .chat-layout { grid-template-columns: 80px 1fr; }
            .sidebar-title span, .case-info, .sidebar-header a { display: none; }
            .sidebar-header { justify-content: center; padding: 20px 0; }
            .case-item { justify-content: center; padding: 12px; }
            .case-avatar { width: 42px; height: 42px; }
...
<div class="chat-layout">
    ...
    <aside class="sidebar">
        <div class="sidebar-header">
            ...
            <a href="{{ route('dashboard') }}" style="color:rgba(255,255,255,0.5);font-size:0.9rem;text-decoration:none;transition:0.3s;" onmouseover="this.style.color='#fff'" onmouseout="this.style.color='rgba(255,255,255,0.5)'">
    ...
    <main class="chat-area">
        @if(isset($activeConversation))
                                ...
                                @foreach($msg->attachments as $att)
                                    <div class="attachment">
                                        ...
                                        <div class="file-details">
                                            ...
                                            <span class="file-size">{{ isset($att['size']) ? round($att['size']/1024, 1).' KB' : 'فایل سند' }}</span>
                                        </div>
                                    </div>
                                @endforeach
                            @endif
            ...
            <footer class="chat-footer">
                <form method="POST" action="{{ route('client.chat.send', $activeConversation->id) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="input-wrapper">
                        ...
                        <input type="text" name="message" class="msg-input" placeholder="پیام خود را تایپ کنید..." autocomplete="off">
